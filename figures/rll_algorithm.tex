	...
	\begin{algorithm}[H]
		\begin{mdframed}[backgroundcolor=green!10,linecolor=white,innerleftmargin=25pt,leftmargin=-25pt,rightmargin=15pt]
			...
			\nl\While{there exist parallel demands $d_{i,j}, d_{u, v}$}{
				...
				Route $d_{i, j}$ or $d_{u, v}$ to miss the cut\;
				Lower capacities accordingly and recompute demands across cuts $D_{i, j}$\;
			}
		...
		\end{mdframed}
		\caption{Schrijver et al.'s \cite{schrijver99} algorithm for computing a minimal solution to an instance of \RRL in its original form.}
		\label{algo:rrl}
	\end{algorithm}
\end{figure}