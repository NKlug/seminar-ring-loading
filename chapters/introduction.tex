...
\begin{center}
	\begin{mdframed}
		...
		\begin{tabular}{rl}
			{\bfseries Input}: & Ring size $n \in \N$ and demands $d_{i, j} \in \R_{\geq 0}$ for all $1 \leq i<j\leq n$.\\
...
One such algorithm was presented by \citet{schrijver99}.
This algorithm achieves a ringload of $L \leq L^{\mathrm{opt}} + \frac{3}{2}D$, where $L^{opt}$ is the optimal ringload and $D$ the maximal demand.
The basic idea behind the algorithm is to first determine a special kind of optimal real routing which all but at most $\lfloor \frac{n}{2} \rfloor$ demands sent both ways around the ring.
This routing is then transformed into an approximation of an optimal binary routing.

In this work, the algorithm is presented and its correctness proven.
This involves two steps: First, deriving an algorithm for computing an optimal real routing and second, transforming it into a binary routing.
...
This runtime is almost optimal, as the encoding length of an instance is in $\Theta(k)$ and in the worst case $k = \binom{n}{2} \in \Theta(n^2)$.
