...
\begin{proof}	
	...
	As we have seen in \cref{cor:ringload}, the optimal ringload is $L^\ast = \frac{M}{2}$.
...
For that, we make the following observation.
\begin{lemma}
	Let $d_{i, j}$ be parallel to a tight cut $\{g, h\}$.
	Then $d_{i, j}$ must be routed to miss $\{g, h\}$.
\end{lemma}
\begin{proof}
	s
\end{proof}
