...
\begin{proof}
	...
	We first show that no edge $\{k, k+1\}$ that lies in between two parallel demands $d_{i, j}, d_{u, v}$ can carry traffic from both demands.
	To see this, assume the contrary and let $\alpha$ and $\beta$ be the amounts of traffic from $d_{i,j}$ and $d_{u, v}$, respectively, that are routed through $\{k, k+1\}$.
	...
	In total, the existence of the routing $\Phi'$ contradicts the minimality of $\Phi^\ast$.
\end{proof}
...
\begin{proof}[Proof of \cref{theo:number-of-splits}]
	Let $\Phi^\ast$ be a minimal routing and let $B$ be the set of demands that are split by $\Phi^\ast$.
	Therefore, we have shown that in a minimal routing and any two
\end{proof}

Remember that in general, we have up to $\binom{n}{2} = \frac{n(n-1)}{2}$ demands.
This means \cref{theo:number-of-splits} implies that in any minimal routing, most demands are either routed entirely through the front or through the back.
As we will see in \cref{sec:ring-loading}, starting from such a routing, it is easy to obtain an approximation to the optimal binary solution.
